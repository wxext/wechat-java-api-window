...
public class DatConvertor {
	...
	public String convert(String path, String output_path) {
		...
		Object[] xor = getXor(fr);
		if (x.get() == 0 && xor[1] != null && (int) xor[1] != 0) {
			x.set((int) xor[1]);
		}
		xor[1] = xor[1] == null ? x.get() : xor[1];
		try {
			InputStream reader = new FileInputStream(fr);
			output = output_path + File.separator + fr.getName().split("\\.")[0] + (xor[0] != null ? "." + xor[0] : "");
			...
			// System.out.println(fr.getName() + "(大小:" + ((double) fr.length()
			// / 1000) + "kb,异或值:" + xor[1] + ")");
			return output;
		} catch (Exception e) {
	...
	private final Map<String, String> FILE_TYPE_MAP = new HashMap<String, String>();
